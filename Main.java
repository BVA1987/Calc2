...
//    }
    ...
    public static String calc (String str) throws Exception {
            String[] goInMassive = str.split(" ");
            int[] goInMassive2 = new int[goInMassive.length];
            char arth = '0';


                    if (goInMassive.length > 3) throw new Exception("Формат математической операции не удовлетворяет заданию - два операнда и один оператор (+, -, /, *)");


                    if (goInMassive.length < 3) throw new Exception("Строка не является математической операцией");
                ...
                } else throw new IllegalArgumentException("Не верный ввод");
